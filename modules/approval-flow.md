...
options.AddFlowCode<Invoice>("Invoice", opt =>
{
    ...
    // 覆寫「怎麼依 objectId 撈出 model」(預設用主鍵查)
    opt.SetGetDataPipe(x => x.Pipe((DbContext db, string id) => db.Set<Invoice>().FindByKey(id)));
});
```
...
1. 前端對某筆 `objectId` 取得**可用動作**:引擎列出該筆資料各條進行中歷程的當關動作,並依**出現條件**過濾;若還沒有歷程,則列出各啟用流程的 **Start 關**動作(= 送單)。
...
- **取可用動作**時過濾:只回傳你**送過單的**、或**當關有指派給你**的、或當下有可按動作的歷程——這是可見性的主要關卡。每筆回傳會附一段該歷程當關的 **StateCode**(加密的歷程識別)。
- **送出動作**時驗的是:StateCode 解得開、動作屬當關且啟用、狀態未被別人推進、你**尚未回應**。**注意:送出端並不再次檢查你是否為當關指派人**——保護是間接的:沒先從 GET 端拿到合法 StateCode 就送不出來,而 StateCode 無法偽造。
...
- **指派為空會擋**:分流到一個非 End 的關卡卻算不出任何指派對象,引擎會丟例外(避免產生無人可簽的死關)——通常是 user/group provider 回空或設計器漏指派所致。
